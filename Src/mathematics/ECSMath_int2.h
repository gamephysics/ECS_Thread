...
#pragma pack(push,1)
	...
	public:
        ...
        static const  int2	    negative_one;
        static const  int2	    npos;
    ...
    __declspec(selectany) const int2    int2::min   = int2(INT_MIN, INT_MIN);
    __declspec(selectany) const int2    int2::one   = int2(1, 1);
    __declspec(selectany) const int2    int2::negative_one = int2(-1, -1);
    __declspec(selectany) const int2    int2::npos  = int2(INT_MIN, INT_MIN);
...
{
	template <>
	struct hash<ecs::int2>
	{
		std::size_t operator()(const ecs::int2& v) const noexcept
		{
            size_t seed = static_cast<size_t>(v.x) << 32 | static_cast<size_t>(v.x);
			return seed;
		}
	};

    template <>
	struct std::equal_to<ecs::int2>
    {
		bool operator()(const ecs::int2& p1, const ecs::int2& p2) const 
        {
			return p1.Equals(p2);
		}
	};
