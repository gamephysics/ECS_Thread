...
{
	template <>
	struct hash<ecs::uint2>
	{
		size_t operator()(const ecs::uint2& v) const noexcept
		{
	...
	template <>
	struct std::equal_to<ecs::uint2>
	{
		bool operator()(const ecs::uint2& p1, const ecs::uint2& p2) const
		{
			return p1.Equals(p2);
		}
	};
