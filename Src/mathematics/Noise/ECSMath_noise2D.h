...
namespace ecs
{
	// https://github.com/Unity-Technologies/Unity.Mathematics/tree/master/src/Unity.Mathematics/Noise/noise2D.cs
	namespace noise
	{
		//=========================================================
		//	
		...
		static float snoise(float2 v)
		{
			...
			float3 m = math::max(0.5f - float3(math::dot(x0, x0), math::dot(x12.xy(), x12.xy()), math::dot(x12.zw(), x12.zw())), 0.0f);
			m = m * m;
			m = m * m;
