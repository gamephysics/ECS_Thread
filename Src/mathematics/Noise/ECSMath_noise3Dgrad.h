...
namespace ecs
{
	// https://github.com/Unity-Technologies/Unity.Mathematics/tree/master/src/Unity.Mathematics/Noise/noise3Dgrad.cs
	namespace noise
	{
		/// <summary>
		/// Simplex noise.
		...
		static float snoise(float3 v, OUT float3& gradient)
		{
			...
			float4 sh = -math::step(h, float4(0.0f));
			...
			float4 m = math::max(0.6f - float4(math::dot(x0, x0), math::dot(x1, x1), math::dot(x2, x2), math::dot(x3, x3)), 0.0f);
			float4 m2 = m * m;
			float4 m4 = m2 * m2;
			float4 pdotx = float4(math::dot(p0, x0), math::dot(p1, x1), math::dot(p2, x2), math::dot(p3, x3));
