...
namespace ecs
{
	// https://github.com/Unity-Technologies/Unity.Mathematics/tree/master/src/Unity.Mathematics/Noise/psrdnoise2D.cs
	namespace noise
	{
		/// <summary>
		...
		static float3 psrdnoise(float2 pos, float2 per, float rot)
		{
			...
			float3 xw = math::fmod(float3(p0.x, p1.x, p2.x), per.x);
			float3 yw = math::fmod(float3(p0.y, p1.y, p2.y), per.y);
			...
			float3 w = float3(math::dot(g0, d0), math::dot(g1, d1), math::dot(g2, d2));

			// Radial weights from corners
			// 0.8 is the square of 2/math.sqrt(5), the distance from
			// a grid point to the nearest simplex boundary
			float3 t = 0.8f - float3(math::dot(d0, d0), math::dot(d1, d1), math::dot(d2, d2));
		...
		/// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
		/// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
		static float3 psrdnoise(float2 pos, float2 per)
		{
		...
		static float psrnoise(float2 pos, float2 per, float rot)
		{
		...
		/// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
		/// <returns>Noise value.</returns>
		static float psrnoise(float2 pos, float2 per)
		{
		...
		/// <param name="rot">Angle to rotate the gradients.</param>
		/// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
		static float3 srdnoise(float2 pos, float rot)
		{
			// Offset y slightly to hide some rare artifacts
			pos.y += 0.001f;
			// Skew to hexagonal grid
		...
		/// <param name="pos">Input (x,y) coordinate.</param>
		/// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
		static float3 srdnoise(float2 pos)
		{
		...
		/// <param name="rot">Angle to rotate the gradients.</param>
		/// <returns>Noise value.</returns>
		static float srnoise(float2 pos, float rot)
		{
			// Offset y slightly to hide some rare artifacts
			pos.y += 0.001f;
			// Skew to hexagonal grid
		...
		/// <param name="pos">Input (x,y) coordinate.</param>
		/// <returns>Noise value.</returns>
		static float srnoise(float2 pos)
		{
