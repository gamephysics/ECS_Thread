...
namespace ecs
{
	// https://github.com/Unity-Technologies/Unity.Mathematics/tree/master/src/Unity.Mathematics/Noise/noise4D.cs
	namespace noise
	{
		/// <summary>
		/// Simplex noise.
		...
		static float snoise(float4 v)
		{
			...
			// First corner
			float4 i = math::floor(v + math::dot(v, float4(F4)));
			...
			// Normalise gradients
			float4 norm = taylorInvSqrt(float4(math::dot(p0, p0), math::dot(p1, p1), math::dot(p2, p2), math::dot(p3, p3)));
			p0 *= norm.x;
			p1 *= norm.y;
			p2 *= norm.z;
			p3 *= norm.w;
			...
			float3 m0 = math::max(0.6f - float3(math::dot(x0, x0), math::dot(x1, x1), math::dot(x2, x2)), 0.0f);
			float2 m1 = math::max(0.6f - float2(math::dot(x3, x3), math::dot(x4, x4)), 0.0f);
			m0 = m0 * m0;
			m1 = m1 * m1;
			return 49.0f * (math::dot(m0 * m0, float3(math::dot(p0, x0), math::dot(p1, x1), math::dot(p2, x2)))
				+ math::dot(m1 * m1, float2(math::dot(p3, x3), math::dot(p4, x4))));
