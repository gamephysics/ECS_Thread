...
namespace ecs
{
	// https://github.com/Unity-Technologies/Unity.Mathematics/tree/master/src/Unity.Mathematics/Noise/common.cs
	//=========================================================
	//
	//=========================================================
	/// <summary>
	...
	namespace noise
	{
		...
		static float4 grad4(float j, float4 ip)
		{
			...
			float3 pxyz = math::floor(math::frac(float3(j) * ip.xyz()) * 7.0f) * ip.z - 1.0f;
		...
		static float2 rgrad2(float2 p, float rot)
		{
			...
			return float2(math::cos(u), math::sin(u));
		}
	}
}
